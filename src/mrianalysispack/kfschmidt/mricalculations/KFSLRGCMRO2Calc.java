...
public class KFSLRGCMRO2Calc extends MRICalculation {
	...
	public void doCalculation() {
		...
		try {
				...
				for (int y = 0; y < height; y++) {
					for (int x = 0; x < width; x++) {
						...
						for (int a = 0; a < co2_stop - co2_start; a++) {
							co2_aslw_array[a] = aslw[co2_start + a][s][x][y] / baseline_perf; // temp
																								// store
																								// F/Fo
							if (co2_aslw_array[a] < 0)
								co2_aslw_array[a] = 0.000001d; // noise

							co2_aslw_array[a] = 1d - Math.pow(co2_aslw_array[a], (alpha - beta));

							co2_bold_array[a] = bold[co2_start + a][s][x][y] / baseline_bold; // temp
																								// store
																								// B/Bo
							if (co2_bold_array[a] < 0)
								co2_bold_array[a] = 0.000001d; // noise

							co2_bold_array[a] = Math.log(co2_bold_array[a]);
						}

						// linear regress to w/o constraint to fit C and M
						double[] regressed = LinearRegression.regress(co2_aslw_array, co2_bold_array);
			...
			ijada.takeImage("LGR_B", c2_map);
			ijada.takeImage("LGR_Rsq_m", r_map);
			ijada.takeImage("LGR_M", m_map);
			aslw = null;
			bold = null;
			System.gc();
		...
		try {
			...
			setStatusMsg("Calculating CMRO2 map");
			int slices = bold[0].length;
			int width = bold[0][0].length;
			int height = bold[0][0][0].length;
				...
				for (int y = 0; y < height; y++) {
					for (int x = 0; x < width; x++) {
						...
						cmro2_map[0][s][x][y] = Math.pow(
								(1d - (stim_bold / baseline_bold * m_map[0][s][x][y])) / Math.pow((stim_perf / baseline_perf), (alpha - beta)), (1d / beta));
			...
			// send the images back to imagej
			ijada.takeImage("LGR_CMRO2", cmro2_map);
			aslw = null;
			bold = null;
			System.gc();
	...
	public String getDisplayTitle() {
		return "Blind Lin. Reg. CMRO2 Calc";
	}
}
