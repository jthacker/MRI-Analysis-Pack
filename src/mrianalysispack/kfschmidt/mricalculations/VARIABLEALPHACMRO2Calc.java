...
public class VARIABLEALPHACMRO2Calc extends MRICalculation {
	...
	public void doCalculation() {
		...
		try {
			...
			alpha_map = new double[1][slices][width][height];
				...
				for (int y = 0; y < height; y++) {
					for (int x = 0; x < width; x++) {
						...
						for (int a = 0; a < co2_stop - co2_start; a++) {
							co2_aslw_array[a] = aslw[a + co2_start][s][x][y] / baseline_perf;
							co2_bold_array[a] = bold[a + co2_start][s][x][y] / baseline_bold;
							if (co2_aslw_array[a] < 0)
								co2_aslw_array[a] = 0d; // noise
							if (co2_bold_array[a] < 0)
								co2_bold_array[a] = 0d; // noise
						}

						// do the regression here
						double[] regressed = ver2regressor.regressHypercapnicVariableAlpha(co2_aslw_array, co2_bold_array, alpha, beta);

						m_map[0][s][x][y] = regressed[0];
						if (m_map[0][s][x][y] < 0)
							m_map[0][s][x][y] = 0d;

						alpha_map[0][s][x][y] = regressed[1];
						if (alpha_map[0][s][x][y] < 0)
							alpha_map[0][s][x][y] = 0d;

						r_map[0][s][x][y] = regressed[2];
			...
			ijada.takeImage("VAR_ALPHA_M", m_map);
			ijada.takeImage("VAR_ALPHA_ALPHA", alpha_map);
			ijada.takeImage("VAR_ALPHA_R", r_map);
			aslw = null;
			bold = null;
			System.gc();
		...
		try {
			...
			for (int s = 0; s < slices; s++) {
				...
				for (int y = 0; y < height; y++) {
					...
					for (int x = 0; x < width; x++) {
						...
						cmro2_map[0][s][x][y] = Math.pow(
								(1d - (stim_bold / baseline_bold * m_map[0][s][x][y])) / Math.pow((stim_perf / baseline_perf), (alpha_map[0][s][x][y] - beta)),
								(1d / beta));
			...
			// send the images back to imagej
			ijada.takeImage("VAR_ALPHA_CMRO2", cmro2_map);
			aslw = null;
			bold = null;
			System.gc();
	...
	public String getDisplayTitle() {
		return "Variable Alpha CMRO2 Calc";
	}
}
