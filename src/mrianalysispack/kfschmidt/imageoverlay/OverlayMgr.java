...
public class OverlayMgr {
	...
	Overlay[] mOverlay; // max 10 slices displayed
	ImageSink mSink;
	int mDisplayMode;
	...
	public Overlay getOverlay(int a) {
		if (mPortSliceDisplayPlan == null)
			return null;
		if (a > mOverlay.length || a < 0)
			return null;
		return mOverlay[a];
	}
	...
	public void showLegend() {
		...
		if (mLegendViewer != null)
			mLegendViewer.setVisible(true);
	}
	...
	public void showViewer() {
		...
		if (mViewer != null && getOverlay(0) != null)
	...
	public float getBgBrightness() {
		if (mOverlay != null && mOverlay.length > 0)
			return mOverlay[0].getBgBrightness();
		else
			return 1f;
	}
	...
	public float getBgContrast() {
		if (mOverlay != null && mOverlay.length > 0)
			return mOverlay[0].getBgContrast();
		else
			return 0f;
	}

	// user actions on control panel
	...
	public void userChangedSliceDisplay(String display_string) {
		// get the current background and maps if avl
		int bg_id = 0;
		int map_id = 0;
		if (mOverlay != null && mOverlay[0].getBackgroundImgId() != 0)
			bg_id = mOverlay[0].getBackgroundImgId();
		if (mOverlay != null && mOverlay[0].getMapImgId() != 0)
			map_id = mOverlay[0].getMapImgId();

		try {
			mCurrentSliceDisplayString = display_string;

			// interpret the string, and make any nec. corrections
			mPortSliceDisplayPlan = interpretSliceDisplayString(display_string);

			// create an overlay for each port
			...
			for (int a = 0; a < mPortSliceDisplayPlan.length; a++) {
				...
				if (bg_id != 0)
					mOverlay[a].setBackgroundImgId(bg_id);
				if (map_id != 0)
					mOverlay[a].setMapImgId(map_id);
			}

			// set the number of ports in the viewer, and let each port call
			// in on repaint
	...
	/**
	 * returns an array of up to 10 ints; array size specifies the number of
	 * view ports open, and the content of each element is the 1 based slice
	 * number that is displayed in the viewport at that index
	 */
	...
	public void repaintMaps() {
		// update the map image on all existing overlays
		for (int a = 0; a < mOverlay.length; a++) {
	...
	public void userChangedBgImage(int new_id) {
		...
		if (mOverlay == null) {
			String str = "1-";
			// if no overlay display plan is present, propose
			// an initial string based on bg image slices
			if (dims[2] > 8)
				str += "8";
			else if (dims[2] == 1)
				str = "1";
		...
		// update the bg image on all existing overlays
		for (int a = 0; a < mOverlay.length; a++) {
	...
	public void sendImagesToIJ() {
		String name = "";
		if (getOverlay(0).getMapImgId() != 0)
			name = mIJada.getNameForImageId(getOverlay(0).getMapImgId());
		else
			name = mIJada.getNameForImageId(getOverlay(0).getBackgroundImgId());
		if (name.indexOf(".") > -1) {
